...
export class Shadow {
  ...
  private static getDefaults(data: IOSData | AndroidData) {
    return Object.assign({}, data, {
      ...
      shadowColor: (data as IOSData).shadowColor || Shadow.DEFAULT_SHADOW_COLOR,
  ...
  private static applyOnAndroid(tnsView: any, data: AndroidData) {
    ...
    if (overrideBackground) {
      if (Shadow.isShadow(currentBg)) {
        ...
        currentBg =
          currentBg instanceof LayeredShadow // if layered, get the original background
      ...
      if (data.shape !== ShapeEnum.RECTANGLE || data.bgcolor || !currentBg) {
        ...
        shape.setShape(android.graphics.drawable.GradientDrawable[data.shape as Shape]);
    ...
    nativeView.setElevation(Shadow.androidDipToPx(nativeView, data.elevation as number));
    nativeView.setTranslationZ(Shadow.androidDipToPx(nativeView, data.translationZ as number));
  ...
  private static overrideDefaultAnimator(nativeView: any, data: AndroidData) {
    ...
    notPressedSet.playTogether(
      java.util.Arrays.asList([ObjectAnimator.ofFloat(nativeView, 'translationZ', [z]).setDuration(buttonDuration), ObjectAnimator.ofFloat(nativeView, 'elevation', [elevation]).setDuration(0)])
    );
